...
class CustomerControllerTest {
    ...
    void getAllCustomers_returnsList() throws Exception {
        Mockito.when(customerService.getAllCustomers()).thenReturn(List.of(new Customer()));
    ...
    void getCustomerById_validId_returnsCustomer() throws Exception {
        Mockito.when(customerService.getCustomerById(1)).thenReturn(Optional.of(new Customer(1, "Jane", "Doe", "jane@example.com", null)));
    ...
    void getCustomerById_notFound_returns404() throws Exception {
        Mockito.when(customerService.getCustomerById(999)).thenReturn(Optional.empty());
    ...
    @Test
    void searchCustomer_byFirstName_returnsMatch() throws Exception {
        Mockito.when(customerService.search(eq("Alice"), any()))
                .thenReturn(List.of(new Customer(1, "Alice", "Smith", "alice@example.com", null)));

        mockMvc.perform(get("/customers/search?firstName=Alice"))
                .andExpect(status().isOk())
    ...
    void createCustomer_blankFirstName_returns400() throws Exception {
        Customer customer = new Customer(null, "", "Doe", "email@example.com", null);
    ...
    void handleUnhandledException_returns500() throws Exception {
        Mockito.when(customerService.getAllCustomers()).thenThrow(new RuntimeException("Unexpected"));
